...
bok_choy_place[bok_choy_name] = favorite_place[index]    #enter the hash, create a key called "Brynne", set value to the first element in the favorite_place array
 ...
 puts "#{bok_choy_name}'s place to visit is #{bok_choy_place[bok_choy_name]}" #interpolate bok_choy_name, then we enter the hash and search for the value associated the bok_choy_name
end